import React, { useEffect, useState } from 'react'
import { useDispatch } from 'react-redux'
import { fetchJobs } from '../../redux/jobs/jobService'
import { CustomButton } from '../UI/CustomButton'

export const Search = () => {
    const [search, setSearch] = useState('')
    const [result, setResult] = useState('')
    const dispatch = useDispatch()
    useEffect(() => {
        if (result) {
            dispatch(fetchJobs(result))
            setSearch('')
        } else {
            dispatch(fetchJobs())
        }
    }, [dispatch, result])

    setTimeout(() => {
        if (result) {
            setResult('')
        }
    }, 8000)

    const handleSubmit = (e) => {
        e.preventDefault()
        if (search.trim !== '') {
            return setResult(search)
        }
    }
    return (
        <form action="" className="mx-auto container" onSubmit={handleSubmit}>
            <div className="relative border-2 border-gray-100 m-4 rounded-lg">
                <div className="absolute top-4 left-3">
                    <i className="fa fa-search text-gray-400 z-20 hover:text-gray-500"></i>
                </div>
                <input
                    type="text"
                    name="search"
                    className="h-14 w-full pl-10 pr-20 rounded-lg z-0 focus:shadow focus:outline-none"
                    placeholder="Search Laravel Gigs..."
                    onChange={(e) => setSearch(e.target.value)}
                    value={search}
                />
                <div className="absolute top-2 right-2">
                    <CustomButton
                        type="submit"
                        className="h-10 w-20 text-white rounded-lg bg-red-500 hover:bg-red-600"
                        text={'Search'}
                    />
                </div>
            </div>
        </form>
    )
}
